...
def get_every_nth_state(states=states, n=10):
    ...
    return [states[i] for i in range(len(states)) if ((i+1) % n) == 0]
...
def get_longest_state(data):
    ...
    # else:
    #   data = sorted(data.keys(), key=len, reverse=True)
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    us_state_abbrev = sorted(us_state_abbrev.values())
    states.sort()
    for i in range(len(us_state_abbrev)):
      if i < 10:
        new_list.append(us_state_abbrev[i])
